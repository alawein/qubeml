...
export GITHUB_REPOSITORY="alawein/qubeml"   # optional; defaults to alawein/qubeml
node scripts/github-sync-report.mjs
```

Artifact: `reports/sync-report.qubeml.json` (gitignored). In **GitHub Actions**, run workflow **"Ops — GitHub sync report"** (workflow_dispatch); it uploads the JSON as a workflow artifact.
